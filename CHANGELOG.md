...
- **`benches/bench_diagnostics.py`** — how long the editor waits.
  ...
  **This server is the outlier, and the benchmark exists to say so.** At
  0.86 ms per record it leaves the "instant" band at roughly 116 records
  and takes over half a second per keystroke on a 500-record document —
  an editor that feels broken. `pain001-lsp` does the same shape of work
  at **0.08 ms per record**, about ten times cheaper, which says the cost
  is specific to this server rather than inherent to the task.

  Nothing is changed here to address it: that is a performance fix with
  its own risks, and it deserves its own change rather than being
  smuggled into a release that adds a benchmark. The number is now
  measured, published and watched.
...
- **The `camt053` floor moves to `>=0.0.17`.** 0.0.17 is the first
  release built against `xmlschema >=4.3.2`. A resolver left free to
  pick 0.0.16 would install xmlschema 3 alongside packages that now
  need 4, and the conflict surfaces at import rather than at install.
...
- **click 8.1.7 → 8.4.2** (PYSEC-2026-2132 / CVE-2026-7246) — bumped the
  transitive `click` pin past the vulnerable range (fixed in >=8.3.3) by
  updating `camt053` 0.0.9 → 0.0.13 in `poetry.lock` and regenerating the
  hash-pinned `requirements/{fuzz,lint,test}.txt` (also picks up
  `markupsafe` 2.1.5 → 3.0.3 as a transitive update). Resolves OpenSSF
  Scorecard Vulnerabilities alert #57.
