...
Measured across document sizes, because a reversing-entry document is not one
...
def build(records: int) -> str:
    """A document carrying ``records`` valid reversing-entry records."""
    rows = []
    for i in range(records):
        row = dict(_RECORD)
        row["entry_ref"] = f"RVSL-NTRY-{i:05d}"
        rows.append(row)
...
def corrupt(text: str) -> str:
    ...
    return text.replace('"amount"', '"amount', 1)
...
ON_DEMAND = [
    ...
    ("hover_text", lambda: lsp_server.hover_text("account_servicer_bic")),
]
